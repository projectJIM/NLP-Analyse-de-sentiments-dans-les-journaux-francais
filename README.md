...
En cliquant sur la **Recherche Entreprise**, l'utilisateur peut faire défiler la liste des entreprises se trouvant sur la partie supérieure de l'interface à l'aide de sa souris ou bien de son clavier, et afin de visualiser le PDG de l'entreprise ainsi que le nombre d'articles considérés comme positif ou négatif, il lui suffit d'entrer le nom de l'entreprise dans la barre de recherche.
...
Nous avons utilisé le package NLTK pour faire plusieurs variantes de textes qui seront utilisés plus tard dans l'analyse: sans les mots vides, la ponctuation ou en cherchant la source du mot. 
De plus, dans cette partie nous avons identifié si un article contenait ou non la mention d'une entreprise proche du journal. Dans le cas du Parisien, le groupe LVMH est l'actionnaire majoritaire du journal. Ceci a été fait à l'aide du package spaCy. (Voir **NLP_Parisien.py**)

Voici deux graphiques qui illustrent l'importance de cette étape. Le premier représente la fréquence d'apparition de mots dans un article du Parisien non traité, et le deuxième montre la fréquence dans le cas ou les mots vides ont été enlevés.  
...
On observe que la précision du classifieur est très proche en fonction de la méthode choisie. Mais ces résultats nous montre une tendance: plus le traitement du texte est lourd, plus on change le texte (par exemple en enlevant la ponctuation et les mots vides - type 3) plus notre classifieur sera instable. Or, notre échantillon n'est pas grand, donc l'instabilité peut provenir de ce fait aussi. Finalement, le texte brut a la meilleure performance d'après ces paramètres. 
Voyons un deuxième indicateur informateur de nos résultats, la matrice de confusion:
...
Ceci n'est qu'un exemple, et la matrice de confusion change en reprenant les calculs. Parmi les 24 articles dans notre base test, on observe que dans chaque instance, il y a en général plus de faux négatifs que de faux positifs. L'estimateur peut donc avoir tendance à mettre un label positif plus facilement qu'il ne le devrait avoir. Or, il faut savoir que notre base train-test contenait initialement plus d'articles négatifs, ce qui est certainement un facteur qui a une influence sur ces résultats. 
...
Nous avons réaliser un test simple de notre classifieur qui exclut les mots vides, en reprenant trois expressions communes qui peuvent apparaitre dans un texte et nous avons eu les résultats suivants: (le code se trouve dans la partie "Avis de la phrase ou du mot" de **Naive_Bayes_Parisien.py**)

Expression  |Avis           
--------------|----------            
"Bien"|   Positif (proba de 0.515 vs 0.485)   
"Mal" |   Négatif   (proba de 0.499 vs 0.501)
"Pas si bien que ça" |   Positif (0.588 vs 0.411)

Sachant que nous avons utilisé le classifieur sans mots vides, le résultat n'est pas satisfaisant. "Mal" est apparament apparu dans un peu plus d'articles négatifs que positifs, et la troisième expression, négative, est acceptée comme positive de 8%.
...
L’avantage principal de cet algorithme est sa simplicité et sa facile interprétation: un article est positif s'il contient plus de mots d’articles entraînés positifs que négatifs.
 ...
 Finalement, nous allons appliquer notre classifieur aux articles des entreprises liées au groupe LVMH, pour en déduire si elles reçoivent un traitement de faveur  du journal Le Parisien. Malheureusement, il n'y a que 12 articles dans notre base de données qui remplissent ce critère, donc toute conclusion sur le traitement privilégié sera limité sur ces articles. (Voir fin de **Naive_Bayes_Parisien.py**)
 
 Selon notre classifieur, des douze articles sur les entreprises du groupe LVMH, neuf sont positifs et trois négatifs. Après avoir observé les douze articles, selon nous, la classification est sans faille: tout les articles ont été bien labelisés. 
 Or, un article négatif et un article positif ne sont que des reportages de "Radio Classique" du groupe LVMH, donc pas directement lié à l'image du groupe, mais tout autant bien classé. Deux articles en effet rapportent des faits négatifs sur le groupe, touchant à des cas de poursuites judiciaires sans fin contre des dirigeants. Finalement, tous les autres articles sont positifs, de manière assez prononcée, en parlant surtout des actions bénéfiques du groupe sur la société.
